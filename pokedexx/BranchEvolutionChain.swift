...
import UIKit
protocol BranchEvolutionChainDelegate : class {
    func didTapPokemonBranch(id: Int32)
}

@IBDesignable class BranchEvolutionChain: UIView {
    ...
    weak var delegate:BranchEvolutionChainDelegate?
    ...
    func tapped(_ sender:UITapGestureRecognizer){
        ...
        if vw.itemID != -1 { //tapped pokemon is not the current
            self.delegate?.didTapPokemonBranch(id: vw.itemID)
        }
    }
